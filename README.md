...
------- | ----------| ---------------------------- | ---------------------------|
 ...
 | TC2: When the game starts, the player is dealt two cards | Run test with only one card in the player_hand while asserting the total length should be two. | Deal another card to the player_hand and run test again. | Yes
 ...
 | TC5:When a players score is updated or evaluated, when it is 21 or less then they have a valid hand | Change the assertFalse method in the test_validity_of_hand test to True  | Assess validity by asserting whether the hand value is True or False. If the hand value is > 21 then the hand is bust and the validity is False | Yes
